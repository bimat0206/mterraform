...
# -----------------------------------------------------------------------------
locals {
  # Service name defaults to 'sqlserver' if not provided
  _service = coalesce(var.service, "sqlserver")
...
resource "aws_security_group" "this" {
  ...
  name        = "${local.name}-sg"
  description = "Security group for ${local.name} SQL Server RDS"
  vpc_id      = var.vpc_id
...
resource "aws_vpc_security_group_ingress_rule" "cidr" {
  ...
  description       = "SQL Server access from ${each.value}"
...
resource "aws_vpc_security_group_ingress_rule" "sg" {
  ...
  description                  = "SQL Server access from security group"
...
resource "aws_secretsmanager_secret" "db_connection" {
  ...
  description             = "Complete connection information for ${local.name} SQL Server database"
