...
variable "service" {
  type        = string
  default     = ""
  description = "Service name (e.g., web, api). If empty, will use 'ec2-windows'"
}
...
variable "instance_type" {
  type        = string
  default     = "t3.medium"
  description = "EC2 instance type (Windows typically needs at least t3.medium)"
}

variable "ami_id" {
  type        = string
  default     = ""
  description = "AMI ID for Windows Server. If empty, will use latest Windows Server 2022 Base"
}
...
variable "get_password_data" {
  type        = bool
  default     = false
  description = "Retrieve Windows administrator password (requires key_name)"
}

variable "user_data" {
  type        = string
  default     = ""
  description = "User data script for instance initialization (PowerShell or batch)"
}
...
variable "security_group_ingress_rules" {
  ...
  default = [
    ...
    },
    {
      ...
      protocol    = "tcp"
      cidr_blocks = ["0.0.0.0/0"]
      description = "WinRM HTTP/HTTPS from anywhere"
    }
  ]
