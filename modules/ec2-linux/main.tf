...
# -----------------------------------------------------------------------------
locals {
  # Service name defaults to 'ec2-linux' if not provided
  _service = coalesce(var.service, "ec2-linux")
  ...
  # Determine AMI ID
  ami_id = var.ami_id != "" ? var.ami_id : data.aws_ami.linux[0].id
...
resource "aws_security_group" "this" {
  ...
  name        = "${local.name}-sg"
  description = "Security group for ${local.name} Linux instance"
  vpc_id      = var.vpc_id
